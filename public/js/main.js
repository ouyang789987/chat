$(function() {
  ...
  socket.on('userlist', function(room, users) {
    ...
    for (var i = users.length - 1; i >= 0; i--) {
      if(users[i] === socket.name) {
        userlist.append('<li>' + users[i] + ' <i class="fa fa-keyboard-o hidden" id="room-'+users[i]+'-typing"></i></li>');
      } else {
        userlist.append('<li onClick="createTab(\''+users[i]+'\', false, true);" class="openPrivateConversation">'+users[i]+' <i class="fa fa-keyboard-o hidden" id="room-'+users[i]+'-typing"></i></li>');
      }
    };
  });
  ...
  socket.on('typing', function(room, user) {
    $('#room-'+user+'-typing').removeClass('hidden');
  });

  socket.on('stop typing', function(room, user) {
    $('#room-'+user+'-typing').addClass('hidden');
  });
